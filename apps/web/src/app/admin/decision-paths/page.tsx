...
export default function DecisionPathExplorerPage() {
  ...
  const activeTraceName = useMemo(
    () => selectedTraceName ?? executionsQuery.data?.[0]?.trace_name ?? null,
    [selectedTraceName, executionsQuery.data],
  );

  const detailQuery = useQuery({
    queryKey: ["admin-agent-trace-execution-detail", activeTraceName],
    queryFn: () => getAdminAgentTraceExecutionDetail(activeTraceName as string),
    enabled: Boolean(activeTraceName),
  });
          ...
          <div className="rounded-lg border border-border bg-card p-4 shadow-card">
            ...
            <div className="space-y-3">
              {(executionsQuery.data ?? []).map((item) => {
                const active = item.trace_name === activeTraceName;
                return (
                  <button
                    key={item.trace_name}
                    type="button"
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">{item.agents.join(" → ")}</div>
          ...
          <div className="rounded-lg border border-border bg-card p-4 shadow-card">
            ...
            {!detailQuery.data ? (
              <div className="text-sm text-muted-foreground">
                Select a runtime execution to inspect decision path and routing payloads.
              </div>
            ) : (
